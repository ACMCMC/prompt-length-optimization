...
\begin{figure*}[t]
  ...
  \end{tikzpicture}
  \caption{\textbf{Our approach.} We aim to simultaneously optimize completion likelihood and suffix length by training a small policy that decides when to shrink, grow, or optimize a suffix, paired with a plug-in optimizer and a frozen language model we treat as black boxes.}
  \label{fig:method}
\end{figure*}
...
We seek a suffix \(s\) that makes a frozen language model \(p_\theta\) assign high probability to a fixed target completion while keeping \(s\) as short as possible. Let \(x\) denote the fixed prefix (e.g., a harmful user query) and \(y\) the fixed target completion (e.g., the corresponding harmful response). The optimization problem is
\begin{equation}
...
In discrete mode we run a Greedy Coordinate Gradient (GCG)  \citet{zou2023universal} update on the suffix tokens. At each GCG step: (i) compute gradients of the log-likelihood with respect to the current suffix token positions; (ii) for each position, form a small candidate pool by replacing that token with top-$k$ alternatives suggested by the gradient; (iii) batch-evaluate these candidates; and (iv) keep the best-scoring suffix. This yields a greedy, gradient-guided token search that improves log-likelihood in a few forward/backward passes while policy actions handle length changes.
