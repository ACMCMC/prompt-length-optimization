...
class ContinuousPromptOptimizerWithProjection(BasePromptOptimizer):
    ...
    def _compute_projection_loss(
        self, embeds: torch.Tensor, mask: Optional[torch.Tensor] = None
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        if self.distance_metric == "l2":
            ...
            vocab_embeds_norm = torch.nn.functional.normalize(
                self.vocab_embeds, p=2, dim=-1
            )
            similarities = torch.matmul(
                flat_embeds_norm, vocab_embeds_norm.t()
            )  # [B*L, vocab_size]
        ...
        if self.max_distance_threshold is not None:
            ...
            penalty = torch.where(
                above_threshold, self.max_distance_threshold + excess**2, penalty
            )
        else:
    ...
    def _compute_projection_loss_per_prompt(
        self, embeds: torch.Tensor, lengths: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        masked_distances = torch.where(
            length_mask, min_distances, torch.zeros_like(min_distances)
        )
        per_prompt_losses = masked_distances.sum(dim=-1) / lengths.float().clamp(
            min=1.0
        )  # [B]
    ...
    def inner_optimization_step(
        ...
        if max_active_len == 0:
            return prompt_data, torch.zeros(
                prompt_data.shape[0], dtype=torch.float32, device=prompt_data.device
            )
        ...
        proj_loss, min_distances = self._compute_projection_loss(
            prompt_data, mask=suffix_mask
        )
