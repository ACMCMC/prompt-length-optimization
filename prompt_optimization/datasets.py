...
class ToxicChatDatasetManager:
    ...
    def load_and_split(
        ...
        print(
            f"Filtered to {len(prompts)} unique prompts (length {min_length}-{max_length} chars)"
        )
        ...
        print(
            f"Split sizes: train={len(train_prompts)}, val={len(val_prompts)}, test={len(test_prompts)}"
        )
    ...
    def get_dataset_statistics(
        self, min_length: int = 30, max_length: int = 200
    ) -> Dict:
        ...
        stats = {
            ...
            "filtered_length_stats": {
                ...
                "mean": (
                    sum(len(p) for p in filtered_prompts) / len(filtered_prompts)
                    if filtered_prompts
                    else 0
                ),
            },
